...
def get_view_vectors(image_shape: Tuple[int, int], fovx: Tensor | None = None, x1y1: Tuple[int, int] = None, x2y2: Tuple[int, int] | None = None) -> Tensor:
...
class Undistort:
    ...
    def __call__(self, image: Tensor, p_image: Tensor, down: Tensor = None,
        ...
        if self.p_image is None \
                or not torch.equal(p_image, self.p_image):
